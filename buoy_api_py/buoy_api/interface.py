...
from rclpy.node import Node
# andermi/add_latentdata_surfacevelocities
...
class Interface(Node):
    ...
    def send_door_command(self, open_doors, blocking=True, timeout=None):
        ...
        resp_dict = {}
        self.tf_set_pos_future_.add_done_callback(
            partial(
                self.default_service_response_callback,
                done=done,
                timeout=timeout,
        ...
        if blocking:
            if not done.wait(timeout) or 'except' in resp_dict:
                self.get_logger().error('door command failed')
